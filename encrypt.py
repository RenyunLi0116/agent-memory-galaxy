#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""把完整 graph.json 用密码加密成 docs/graph.enc.json（供公开 Pages 发布）。
方案：PBKDF2-HMAC-SHA256(200k) 派生密钥 + AES-256-GCM，与浏览器 Web Crypto 解密完全兼容。
...
def main():
    ...
    out = os.path.join(HERE, "docs", "graph.enc.json")
    ...
    print("  公开 Pages 将只含此密文。明文 graph.json / standalone.html 不入库。")
